...
function showPrototypeResults(project) {
    ...
    container.innerHTML = `
        ...
        <div style="padding: 20px; background: #f8f9fa; border-radius: 8px; margin-bottom: 20px;">
            ...
            ${project.input_data?.requirement || ''}
        </div>
        ${html ? `
        <div style="margin-bottom: 20px;">
            <h3 style="margin-bottom: 10px;">原型预览</h3>
            <iframe srcdoc="${html.replace(/"/g, '&quot;')}" style="width: 100%; height: 600px; border: 2px solid #e0e0e0; border-radius: 8px;"></iframe>
        </div>
        ` : ''}
        <div style="text-align: center; padding: 20px;">
            <button class="btn btn-primary" onclick="downloadPrototype('${project.id}')">下载HTML</button>
        </div>
    `;
    document.getElementById('processing-page').classList.add('hidden');
    container.classList.remove('hidden');
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 30px;">
            ${designs.map(design => `
                <div style="background: white; border: 2px solid #e0e0e0; border-radius: 12px; overflow: hidden;">
                    <div style="width: 100%; height: 300px; background: #f8f9fa; display: flex; align-items: center; justify-content: center; color: #999;">
                        <iframe srcdoc="${(design.html || '').replace(/"/g, '&quot;')}" style="width: 100%; height: 100%; border: none;"></iframe>
                    </div>
                    <div style="padding: 20px;">
                        ...
                        <div style="display: flex; gap: 10px;">
                            <button class="btn btn-primary" style="flex: 1; padding: 10px;" onclick="viewDesign('${design.id}')">查看</button>
                            <button class="btn" style="flex: 1; padding: 10px; background: #f0f0f0;" onclick="downloadDesign('${design.id}')">导出</button>
                        </div>
                    </div>
                </div>
            `).join('')}
